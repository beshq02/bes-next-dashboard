...
export default async function TenderDetailPage({ params }) {
  ...
  const screenshotUrl = detail
    ? supabase.storage.from('file').getPublicUrl(`gcc_tender_detail/${detail.id}.png`).data.publicUrl
    : null
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <Badge className="bg-bes-blue-500 text-white hover:bg-bes-blue-600">招標公告</Badge>
                  {getProcurementLevelBadge(procurementLevel)}
                  {history.length > 0 && (
                    <Badge variant="secondary">{history.length + 1} 次公告</Badge>
                  )}
                </div>
                    ...
                    <TooltipTrigger asChild>
                      <div className="flex cursor-default items-start gap-3 rounded-lg bg-bes-blue-50/50 p-3">
                        ...
                        <div>
                          ...
                          <p className="text-base font-semibold text-bes-blue-950">{announcement.org_name}</p>
                        </div>
                      </div>
                    </TooltipTrigger>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm sm:grid-cols-5">
              <div>
                ...
                <p className="font-medium">
                  {announcement.transmission_count ? `第 ${announcement.transmission_count} 次` : '-'}
                </p>
              </div>
              <div>
        ...
        {detail ? (
          <>
            {CATEGORY_CONFIG.map((cat) => {
              const hasFields = fieldMapping.some(fm => fm.category === cat.key && fm.field_code !== 'a01')
            ...
            <DetailCard icon={Paperclip} title={`附件檔案 (${tenderFiles.length})`}>
              <FileList
                files={tenderFiles}
                emptyText="此標案尚無附件檔案"
                getDownloadUrl={(file) => {
                  if (file.download_status !== 'success' || !file.storage_path) return null
                  return supabase.storage.from('file').getPublicUrl(file.storage_path, { download: file.file_name || true }).data.publicUrl
                }}
              />
            </DetailCard>
          </>
          ...
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <h3 className="mb-2 font-heading text-lg font-medium text-bes-blue-900">尚無詳細資料</h3>
