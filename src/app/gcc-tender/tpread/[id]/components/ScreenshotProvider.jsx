...
import { X, Camera, ZoomIn, Loader2, ZoomOut, ImageOff } from 'lucide-react'
...
function ScreenshotImage({ src, tenderName, zoom }) {
    ...
    >
      {error ? (
        ...
        <>
          ...
          <img
            src={src}
            alt={`${tenderName} - 政府電子採購網 頁面截圖`}
            className={`max-w-none origin-top-left select-none ${loading ? 'hidden' : ''}`}
            draggable={false}
  ...
  return (
    <ScreenshotContext.Provider value={{ open, setOpen, screenshotUrl, tenderName }}>
      ...
      <div className="lg:flex">
        ...
        {open && isDesktop && (
          <div className="sticky top-0 flex h-screen w-1/2 flex-col border-l border-slate-200 bg-white">
            {/* Panel header — title + zoom controls */}
            <div className="flex shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4 py-2">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-bes-blue-900">頁面截圖</h3>
              </div>
              <div className="flex items-center gap-1">
            ...
            {/* Panel body — no padding, image fills */}
            <div className="min-h-0 flex-1">
              <ScreenshotImage src={screenshotUrl} tenderName={tenderName} zoom={zoom} />
            </div>
          </div>
        )}
      </div>
      ...
      <Sheet open={open && !isDesktop} onOpenChange={setOpen}>
        <SheetContent side="bottom" className="flex h-dvh w-full max-w-full flex-col p-0">
          <SheetHeader className="shrink-0 border-b px-4 py-2 text-left">
            ...
            <SheetDescription className="truncate text-xs">{tenderName}</SheetDescription>
          </SheetHeader>
          <div className="min-h-0 flex-1">
            <ScreenshotImage src={screenshotUrl} tenderName={tenderName} zoom={zoom} />
          </div>
